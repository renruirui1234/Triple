...
class DeepSVDD(object):
    ...
    def __init__(self, opt):
        ...
        assert (0 < opt.nu) & (opt.nu <= 1), "For hyperparameter nu, it must hold: 0 < nu <= 1."
        self.nu = opt.nu
    ...
    def train(self, dataset: BaseADDataset, optimizer_name: str = 'adam', lr: float = 0.001, n_epochs: int = 50,nu:int=0.1,
        ...
        self.trainer = DeepSVDDTrainer(self.objective, self.R, self.c, self.nu, optimizer_name, lr=lr,
                                       ...
                                       weight_decay=weight_decay, device=device, n_jobs_dataloader=n_jobs_dataloader,opt=self.opt)
        # Get the model
        self.net = self.trainer.train(dataset, self.net,self.opt)
    ...
    def test(self, dataset: BaseADDataset, device: str = 'cuda', n_jobs_dataloader: int = 0):
        ...
        f1_score_val,precision_score_val,recall_score_val,scores=self.trainer.test(dataset, self.net)
        # Get results
        self.results['test_auc'] = self.trainer.test_auc
        self.results['test_time'] = self.trainer.test_time
        self.results['test_scores'] = self.trainer.test_scores
        return self.trainer.y_val_pred,f1_score_val,precision_score_val,recall_score_val,scores
